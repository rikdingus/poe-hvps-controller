...
import { Activity, ShieldCheck, Zap, Settings, Gauge, TrendingUp, LayoutDashboard, Cpu, Radio } from 'lucide-react';
...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      const detRes = await fetch('/api/detectors');
      const liveDets = await detRes.json();
      
      const digRes = await fetch('/api/digitizer');
      ...
      if (liveDets && liveDets.length > 0) {
        ...
        setHistory(prev => [...prev.slice(-29), { time: new Date().toLocaleTimeString(), rate: liveDig.triggerRate }]);
      } else {
        // Fallback mock
        ...
        setHistory(prev => [...prev.slice(-29), { time: new Date().toLocaleTimeString(), rate: (Math.random()*5+2).toFixed(2) }]);
      }
    } catch (e) {
      ...
      {isEmergencyStopped && (
        <div className="fixed inset-0 z-50 bg-white/90 backdrop-blur-md flex flex-col items-center justify-center">
          <div className="text-center p-16 border-4 border-[#be2c2e]">
            ...
            <h1 className="text-7xl font-black tracking-tighter text-[#be2c2e] mb-4 uppercase italic">ACQUISITION HALTED</h1>
            <p className="text-gray-500 tracking-widest uppercase text-sm font-bold">Scientific Override Active</p>
            <button onClick={() => setIsEmergencyStopped(false)} className="mt-12 px-12 py-4 bg-[#be2c2e] text-white text-sm font-black uppercase tracking-widest hover:bg-[#7a0000]">Resume Run →</button>
          </div>
        </div>
      )}
            ...
            <div className="h-[150px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={history}>
                  <Area type="monotone" dataKey="rate" stroke="#be2c2e" strokeWidth={3} fill="#be2c2e" fillOpacity={0.05} />
